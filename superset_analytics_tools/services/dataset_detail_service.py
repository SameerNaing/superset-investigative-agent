...
class DatasetDetailService:
    def _get_charts_metrics(self, charts): 
        ...
        for chart in charts:
            ...
            metrics_a, metrics_b = extract_chart_metrics(form_data)
            
            if not metrics_a:
                continue
                        
            metrics.extend(parse_metrics(metrics_a))
    ...
    def get(self, dataset_id):
        ...
        charts = [superset_schemas.ChartList(id=chart.get("id"), name=chart.get("slice_name"), viz_type=chart.get("viz_type")) 
                  for chart in charts 
                  if chart.get("id") not in superset_consts.ignore_chart_ids and 
                     chart.get("viz_type") not in superset_consts.ignore_chart_viz_types]
